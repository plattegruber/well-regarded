...
5. **`queue:classify`** — AI classification via `packages/ai`, writing `derivations` rows (haiku-tier model, zod-validated structured output). Implemented in [#67](https://github.com/plattegruber/well-regarded/issues/67): **one** call per signal returns all four judgments (sentiment, urgency, response risk, publication suitability), each with confidence and a one-line rationale, stored with basis `inferred_text` and the concrete `model_version`. Low-confidence urgency (< 0.5) is floored **up** one level (`applyUrgencyFloor` in `packages/ai`) — a missed urgent complaint is a patient walking away in pain; a false alarm costs a human ten seconds. Rating-only signals (empty or < 3-word text) skip the model entirely and get deterministic judgments from the rating (basis `source_metadata`). **Cost:** ~1 Haiku call per text signal — a backfill of 2,000 historical reviews is 2,000 calls, which is acceptable *because* it drains through the queue's `max_batch_size`/`max_retries` settings instead of slamming the API, with the client's 429 backoff ([#63](https://github.com/plattegruber/well-regarded/issues/63)) absorbing bursts; every call is cost-logged to `ai_calls` under purpose `judgments`, billed to the signal's practice.
