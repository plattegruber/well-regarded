...
| `RAW_IMPORTS`    | R2 bucket       | api, jobs                  | Raw uploaded/imported source files (`wr-raw-imports-<env>`): api writes uploads (#133), the CSV import Workflow reads them (#135) |
| `RAW_ARTIFACTS`  | R2 bucket       | pipeline, jobs             | Immutable content-addressed raw source artifacts (#100), read by the normalize stage; the CSV import Workflow writes batch envelopes here (`wr-raw-artifacts-<env>`) |
...
| `CSV_IMPORT`     | Workflow        | jobs                       | The `wr-csv-import-<env>` Workflow (class `CsvImport`, #135): durable CSV import — validate, batch, enqueue, await drain, finalize — see [csv-import.md](csv-import.md) |
