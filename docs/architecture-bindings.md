...
| `RAW_ARTIFACTS`  | R2 bucket       | pipeline, jobs             | Immutable content-addressed raw source artifacts (#100), read by the normalize stage; the CSV import Workflow (#135) writes batch envelopes and the GBP poller (#123) review-page envelopes here BEFORE enqueueing — store-before-enqueue (`wr-raw-artifacts-<env>`) |
