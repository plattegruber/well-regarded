...
/**
 * Provenance for the CSV-sourced signals. The `import_runs` table itself
 * lands with Epic #6 — until then `signals.import_run_id` is a bare uuid
 * column (no FK), so the seed stamps this deterministic run id on every
 * `csv_import` signal and Epic #6 backfills the run row.
 */
